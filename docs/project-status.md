...
> **Last updated:** 2026-06-21 (Phase 5A-8 Remove Bot Radius Telegraph — Agent A draft)  
> **Base branch:** `claude/game-file-analysis-a20xup` @ `526da54`
...
**Phase 5A-8 — Remove Bot Pre-Attack / Skill Radius Telegraph** — **RUNTIME IN DRAFT (Agent A)**

**Agent A runtime in draft** — branch `cursor/phase-5a-8-remove-bot-radius-telegraph`, base `526da54`. Removes the ground-painted attack/skill radius/cone the product owner does not want: `BotPlayer.showWindupCue` drew a sector spanning `min(attackRange, 130) + radius` in front of the bot for the whole wind-up — a map-filling area read for ranged classes. The sector draw is replaced with a small **fixed** glow (`radius * 0.5`, ~12px) centered on the bot's own body, independent of `attackRange`/`attackArcDegrees` — no radius circle, cone, or range ring remains anywhere in gameplay. The body tint flash (already part of `showWindupCue`) stays as the primary readability signal; normal-attack projectiles, skill VFX (cast flash, distinct skill bolt, heal burst/spark), damage/heal timing, and BotBrain behavior are all unchanged — only the spatial ground-warning graphic was touched. Now-unused `attackRange`/`attackArcDegrees` fields on the `BotPlayer` entity (only ever used by the removed sector math) were deleted along with the call-site values; the combat math in `BotPlayerSystem` (the actual hit-test / damage path) is untouched. New regression `phase-5a-remove-bot-telegraph-regression.mjs` proves the cue stays ≤20px (vs. the old ~89–154px) across all 4 classes for both basic attack and skill cast, while 9 existing suites (incl. the 5A-7 live-class/skill-parity suite, whose checks still expect *some* visible wind-up cue) all stay green. **Not Ready, not merged.** Phase 5B/5C not started.
